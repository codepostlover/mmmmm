...
try:
    ...
    def TestCase():
      ...
      try:
        ...
        msg += f"\nTesting in_edges({idx})..."
        expected_edges = graph.in_edges(idx)
        returned_edges = student_graph.in_edges(idx)
...
f = open("/outputs/103335.json", "w")
json.dump({"id": "103335", "passed": output.passed, "log": output.logs}, f)
f.close()
