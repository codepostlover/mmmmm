...
try:
    ...
    def TestCase():
      ...
      try: 
          ...
          msg += "\nTesting bh.post_order()..."
          expected_trav = expected_heap.post_order()
          student_trav = student_heap.post_order()
          
          msg += "\n\nExpected post-order traversal:\n" + str(expected_trav) 
          msg += "\nReturned post-order traversal:\n" + str(student_trav) 
...
f = open("/outputs/101791.json", "w")
json.dump({"id": "101791", "passed": output.passed, "log": output.logs}, f)
f.close()
