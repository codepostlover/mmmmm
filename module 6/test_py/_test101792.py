...
try:
    ...
    def TestCase():
      ...
      try: 
          ...
          msg += "\nTesting bh.pre_order()..."
          expected_trav = expected_heap.pre_order()
          student_trav = student_heap.pre_order()
          
          msg += "\n\nExpected pre-order traversal:\n" + str(expected_trav) 
          msg += "\nReturned pre-order traversal:\n" + str(student_trav) 
...
f = open("/outputs/101792.json", "w")
json.dump({"id": "101792", "passed": output.passed, "log": output.logs}, f)
f.close()
